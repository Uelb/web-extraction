...
SSQL.processData = function(data, weights) {
  var element, i, labels, root, to_use_distance, vectors;
  if (weights == null) {
    weights = null;
  }
  if (weights) {
    to_use_distance = function(vec1, vec2) {
      return SSQL.CUSTOM_WEIGHTED_DISTANCE(vec1, vec2, weights);
    };
  } else {
...
SSQL.getWeightVector = function(weights) {
  ...
  result = [weights.color, weights.bgColor, weights.width, weights.height, weights.textDecoration, weights.fontStyle, weights.leftAlignment, weights.topAlignement, weights.zIndex, weights.paddingLR, weights.paddingTB, weights.borderHorizontalWidth, weights.borderVerticalWidth];
...
SSQL.getVector = function(element) {
  var bgColor, borderHorizontalWidth, borderVerticalWidth, color, fontStyle, height, leftAlignment, paddingLR, paddingTB, textDecoration, topAlignement, width, zIndex;
  ...
  return [color, bgColor, width, height, textDecoration, fontStyle, leftAlignment, topAlignement, zIndex, paddingLR, paddingTB, borderHorizontalWidth, borderVerticalWidth];
};
